...
//		    dt = *(int16_t*)RX_BUF;
		dt = (uint8_t*)RX_BUF;
		CDC_Transmit_FS((uint8_t*)dt, (uint16_t)strlen((const char*)TX_PLOAD_WIDTH)); 
//    CDC_Transmit_FS((uint8_t)dt, strlen((uint8_t*)dt));
    dt = (uint8_t*)(RX_BUF+2) ;
//				dt = *(uint8_t*)RX_BUF;
		CDC_Transmit_FS((uint8_t*)dt, (uint16_t)strlen((const char*)dt));
